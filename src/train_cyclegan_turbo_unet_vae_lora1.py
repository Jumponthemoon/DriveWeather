...
from cyclegan_turbo_unet_vae_lora import CycleGAN_Turbo, VAE_encode, VAE_decode, initialize_unet, initialize_vae
...
########################################################################
                    for p in vae_enc.vae.lora_params['rainy']:
                        print('1')
                        p.requires_grad = False
                    # 设置 `snowy` LoRA 参数为不可训练
                    for p in vae_enc.vae.lora_params['snowy']:
                        p.requires_grad = True
                        print('2')

                    for p in vae_enc.vae_b2a.lora_params['rainy']:
                        p.requires_grad = False
                        print('3')

                    # 设置 `snowy` LoRA 参数为不可训练
                    for p in vae_enc.vae_b2a.lora_params['snowy']:
                        p.requires_grad = True
                        print('5')

#########################################################################
                    for p in vae_dec.vae.lora_params['rainy']:
                        p.requires_grad = False
                    # 设置 `snowy` LoRA 参数为不可训练
                    for p in vae_dec.vae.lora_params['snowy']:
                        p.requires_grad = True

                    for p in vae_dec.vae_b2a.lora_params['rainy']:
                        p.requires_grad = False
                    # 设置 `snowy` LoRA 参数为不可训练
                    for p in vae_dec.vae_b2a.lora_params['snowy']:
                        p.requires_grad = True
##########################################################################
                else:
                    # print(tgt_weather)
