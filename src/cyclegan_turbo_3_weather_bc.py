...
class UNetWithAttention(nn.Module):
    def __init__(self, unet):
        ...
        self.unet = unet
        self.ref_att = {}  # 存储不同层的注意力特征

    def forward(self, x, timesteps, encoder_hidden_states):
        for i, layer in enumerate(self.unet.down_blocks):
            # 如果该层有 self-attention 模块
            # print(i,layer)

            if hasattr(layer, 'attn1'):
                # 提取并保存 key 和 value 特征

                attention_module = layer.attn1
                self.ref_att[f"layer_{i}_key"] = attention_module.key
                self.ref_att[f"layer_{i}_value"] = attention_module.value
        # print(x.size(),timesteps.size(),encoder_hidden_states.size())
        return self.unet(x, timesteps, encoder_hidden_states)



class CrossAttentionUNet(nn.Module):
    def __init__(self, unet, reference_attention_features):
        super(CrossAttentionUNet, self).__init__()
        self.unet = unet
        self.reference_attention_features = reference_attention_features  # 中间图像的注意力特征

    def forward(self, x, timesteps, encoder_hidden_states):
        for i, layer in enumerate(self.unet.down_blocks):
            print(i,layer)
            if hasattr(layer, 'attn1'):
                # 将中间图像的 key 和 value 替换当前图片的 key 和 value
                attention_module = layer.attn1
                attention_module.key = self.reference_attention_features[f"layer_{i}_key"]
                attention_module.value = self.reference_attention_features[f"layer_{i}_value"]
        return self.unet(x, timesteps, encoder_hidden_states)
...
def initialize_unet(rank, return_lora_module_names=False):
    ...
    # Define target modules for different types
    target_modules = {
        'encoder': l_target_modules_encoder,
        'decoder': l_target_modules_decoder,
        'others': l_modules_others,
    }

    # Define conditions
    conditions = ['rainy', 'snowy', 'foggy']

    # Create and store LoraConfig instances in a dictionary
    lora_configs = {
        condition: {
            adapter_type: LoraConfig(
                r=rank,
                init_lora_weights="gaussian",
                target_modules=target_module,
                lora_alpha=rank
            )
            for adapter_type, target_module in target_modules.items()
        }
        for condition in conditions
    }

    # Add adapters to UNet
    for condition, adapters in lora_configs.items():
        for adapter_type, lora_conf in adapters.items():
            adapter_name = f"{condition}_{adapter_type}"
            unet.add_adapter(lora_conf, adapter_name=adapter_name)
...
class CycleGAN_Turbo(torch.nn.Module):
    def __init__(self, mode = None, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_unet=8, lora_rank_vae=4):
        ...
        vae = AutoencoderKL.from_pretrained("stabilityai/sd-turbo", subfolder="vae")
        unet = UNet2DConditionModel.from_pretrained("stabilityai/sd-turbo", subfolder="unet")
        # unet.set_attn_processor(processor=AttnProcessor())
        vae.encoder.forward = my_vae_encoder_fwd.__get__(vae.encoder, vae.encoder.__class__)
        vae.decoder.forward = my_vae_decoder_fwd.__get__(vae.decoder, vae.decoder.__class__)
        # add the skip connection convs
        ...
        self.middle_image_unet = UNetWithAttention(self.unet)
    ...
    def load_ckpt_from_state_dict(self, sd):
        ...
        # Define the mapping for the different types of LoRA layers
        adapter_types = {
            "default_encoder": ["sd_snowy_encoder", "sd_rainy_encoder", "sd_foggy_encoder"],
            "default_decoder": ["sd_snowy_decoder", "sd_rainy_decoder", "sd_foggy_decoder"],
            "default_others": ["sd_snowy_other", "sd_rainy_other", "sd_foggy_other"],
        }

        # Loop through named parameters in unet and handle each type dynamically
        for n, p in self.unet.named_parameters():
            for adapter_key, sd_keys in adapter_types.items():
                if "lora" in n and adapter_key in n:
                    name_sd = n.replace(f".{adapter_key}.weight", ".weight")
                    blended_weight = (
                        alpha * sd[sd_keys[0]][name_sd]
                        + beta * sd[sd_keys[1]][name_sd]
                        + theta * sd[sd_keys[2]][name_sd]
                    )
                    p.data.copy_(blended_weight)
    ...
    def forward_with_networks(x, x_label, direction, vae_enc, unet, vae_dec, sched, timesteps, text_emb, alpha, mode, middle_image_attention_features=None):
        ...
        if mode == 'train':
            adapter_types = ['encoder', 'decoder', 'others']
            # Create list of adapters using list comprehension
            adapters = [f"{condition}_{adapter_type}" for condition in conditions for adapter_type in adapter_types]
        ...
        if middle_image_attention_features:
            # 如果提供了中间图像的注意力特征，则使用 CrossAttentionUNet
            cross_attention_unet = CrossAttentionUNet(unet, middle_image_attention_features)
            unet = cross_attention_unet

        x_cat = torch.cat((x, x_label), dim=1)
        x_enc = vae_enc(x_cat, direction=direction).to(x.dtype)
        if mode !='train':
            text_emb =text_emb.repeat(5, 1, 1)
    ...
    def forward(self, x_t, x_t_label, alpha, mode=None, direction=None, caption=None, caption_emb=None, middle_image=None):
        ...
        # print(x_t[0].size(),'---')
        middle_image_attention_features = self.extract_middle_image_attention(x_t[0].unsqueeze(0), caption_enc)
        # else:
        #     middle_image_attention_features = None


        return self.forward_with_networks(
            x_t, x_t_label, direction, self.vae_enc, self.unet, self.vae_dec, self.sched, self.timesteps, caption_enc, alpha, mode, middle_image_attention_features)