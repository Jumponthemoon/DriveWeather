...
def main(args):
  ...
  #  params_disc = list(net_disc_a.parameters()) + list(net_disc_b.parameters())
    ...
    unet, vae_enc, vae_dec, net_disc_snow, net_disc_rain, net_disc_b = accelerator.prepare(unet, vae_enc, vae_dec, net_disc_snow, net_disc_rain, net_disc_b)
    ...
    net_lpips, optimizer_gen, optimizer_disc_snow, optimizer_disc_rain, train_dataloader, lr_scheduler_gen, lr_scheduler_disc_snow, lr_scheduler_disc_rain = accelerator.prepare(
    net_lpips, optimizer_gen, optimizer_disc_snow, optimizer_disc_rain, train_dataloader, lr_scheduler_gen, lr_scheduler_disc_snow, lr_scheduler_disc_rain
)
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
           # l_acc = [unet, net_disc_a, net_disc_b, vae_enc, vae_dec]
            l_acc = [unet, net_disc_snow,net_disc_rain, net_disc_b, vae_enc, vae_dec]
            ...
            with accelerator.accumulate(*l_acc):
                ...
                """
                # A -> fake B -> rec A
                cyc_fake_b = CycleGAN_Turbo.forward_with_networks(img_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb,alpha)
                # cyc_fake_b = torch.cat((cyc_fake_b, img_a[:,-1,:,:].unsqueeze(1)), dim=1)
               
                cyc_rec_a = CycleGAN_Turbo.forward_with_networks(torch.cat((cyc_fake_b, img_a[:,-1,:,:].unsqueeze(1)), dim=1), "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb,alpha)
                # cyc_rec_a = CycleGAN_Turbo.forward_with_networks(cyc_fake_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb,alpha)


                loss_cycle_a = crit_cycle(cyc_rec_a, img_a[:,3,:,:]) * args.lambda_cycle
                loss_cycle_a += net_lpips(cyc_rec_a, img_a[:,:3,:,:]).mean() * args.lambda_cycle_lpips
                # B -> fake A -> rec B
                cyc_fake_a = CycleGAN_Turbo.forward_with_networks(img_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb,alpha)
                # cyc_fake_a = torch.cat((cyc_fake_a, img_b[:,-1,:,:].unsqueeze(1)), dim=1)

                cyc_rec_b = CycleGAN_Turbo.forward_with_networks(torch.cat((cyc_fake_a, img_b[:,-1,:,:].unsqueeze(1)), dim=1), "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb,alpha)
                # cyc_rec_b = CycleGAN_Turbo.forward_with_networks(cyc_fake_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb,alpha)

                # cyc_rec_b = torch.cat((cyc_rec_b, cyc_fake_a[:,-1,:,:].unsqueeze(1)), dim=1)

                loss_cycle_b = crit_cycle(cyc_rec_b, img_b[:,3,:,:]) * args.lambda_cycle
                loss_cycle_b += net_lpips(cyc_rec_b, img_b[:,:3,:,:]).mean() * args.lambda_cycle_lpips
                ...
                """
                fake_a = CycleGAN_Turbo.forward_with_networks(img_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb,alpha)
                fake_b = CycleGAN_Turbo.forward_with_networks(img_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb,alpha)
                ...
                Identity Objective
                """
                idt_a = CycleGAN_Turbo.forward_with_networks(img_b, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb,alpha)
                loss_idt_a = crit_idt(idt_a, img_b[:,3,:,:]) * args.lambda_idt
                loss_idt_a += net_lpips(idt_a, img_b[:,3,:,:]).mean() * args.lambda_idt_lpips
                idt_b = CycleGAN_Turbo.forward_with_networks(img_a, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb,alpha)
                loss_idt_b = crit_idt(idt_b, img_a[:,3,:,:]) * args.lambda_idt
                loss_idt_b += net_lpips(idt_b, img_a[:,3,:,:]).mean() * args.lambda_idt_lpips
             ...
             #   loss_D_A_real = net_disc_a(img_b, for_real=True).mean() * args.lambda_gan
                if tgt_weather[0] =='snow':
                    loss_D_A_real = net_disc_snow(img_b[:,3,:,:], for_real=True).mean() * args.lambda_gan
                else:
                    loss_D_A_real = net_disc_rain(img_b[:,3,:,:], for_real=True).mean() * args.lambda_gan


                loss_D_B_real = net_disc_b(img_a[:,3,:,:], for_real=True).mean() * args.lambda_gan
                    ...
                    if global_step % args.viz_freq == 1:
                        for tracker in accelerator.trackers:
                            if tracker.name == "wandb":
                                ...
                                viz_img_a = viz_img_a[:,:3,:,:]
                                viz_img_b = batch["pixel_values_tgt"].to(dtype=weight_dtype)
                                viz_img_b = viz_img_b[:,:3,:,:]
                                print(fake_b.size(),'111111')
                                log_dict = {
