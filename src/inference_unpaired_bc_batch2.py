...
from cyclegan_turbo_3_weather_bc_processor_multi import CycleGAN_Turbo
...
#    # label_path = image_path.replace('test', 'train_A_seg').replace('_frame_camera.png', '_gt_labelIds.png')
...
def save_output_images(output_tensors, input_paths, output_dir,view_length):
 ...
 #   print(output_tensors.size(),'1111')
    for j, output_tensor in enumerate(output_tensors):
      #  print(i)
        ...
        output_pil = output_pil.resize((Image.open(input_paths[idx]).width, Image.open(input_paths[idx]).height), Image.LANCZOS)
        output_pil.save(os.path.join(output_dir, os.path.basename(input_paths[idx])))
        # if j < 5:

        # else:
        #     j = j%5
        #     output_pil = output_pil.resize((Image.open(input_paths[i]).width, Image.open(input_image_paths_nextj[j]).height), Image.LANCZOS)
...
if __name__ == "__main__":
   ...
   # print(frames)
    ...
    model = CycleGAN_Turbo(frame_length,pretrained_name=args.model_name, pretrained_path=args.model_path)
    model.eval()
    if args.use_fp16:
        model.half()
    ...
    #     input_image_paths = [os.path.join(image_root, f'{frame}_{view}.jpg') for view in range(6)]
    #     images, labels = zip(*(load_and_prepare_image(path, T_val) for path in input_image_paths))


    #     input_image_paths_next = [os.path.join(image_root, f'{frame_next}_{view}.jpg') for view in range(6)]
    ...
    #     x_t_label = torch.cat([x_t_label,x_t_label_next],dim=0)
        ...
        # frame_length = len(batches)
        # print(frame_length)
        # view_length = views
        total_length = frame_length*view_length
        print(total_length)
        # 加载并拼接5张图片
        images_concat, labels_concat = load_and_prepare_images(input_image_paths, T_val)

        # 将拼接的图像转换为张量并移动到设备
        x_t = images_concat.to(device)
        x_t_label = labels_concat.to(device)
   ...
   #     print(x_t.size(),x_t_label.size())
        ...
        # Perform inference
        with torch.no_grad():
            output = model(x_t, x_t_label,args.alpha,total_length, direction=args.direction, caption=args.prompt, mode='inference')
        
        # Save the output images
        save_output_images(output, input_image_paths, args.output_dir,view_length)
