...
class CycleGAN_Turbo(torch.nn.Module):
    ...
    def load_ckpt_from_state_dict(self, sd):
        ...
        for n, p in self.unet.named_parameters():
            ...
            if "lora" in n and "default_encoder" in n:
               ...
               # print(p.device)
                ...
                blended_weight = scale*(alpha * sd["sd_snowy_encoder"][name_sd] + (1-alpha) * sd["sd_rainy_encoder"][name_sd] )
        ...
        for n, p in self.unet.named_parameters():
            ...
            if "lora" in n and "default_decoder" in n:
                ...
                blended_weight = scale*(alpha * sd["sd_snowy_decoder"][name_sd] + (1-alpha) * sd["sd_rainy_decoder"][name_sd] )
        ...
        for n, p in self.unet.named_parameters():
            ...
            if "lora" in n and "default_others" in n:
                ...
                blended_weight = scale*(alpha * sd["sd_snowy_other"][name_sd]+ (1-alpha) * sd["sd_rainy_other"][name_sd])
    ...
    @staticmethod
    def forward_with_networks(x,x_label, direction, vae_enc, unet, vae_dec, sched, timesteps, text_emb,alpha):
        B = x.shape[0]
        assert direction in ["a2b", "b2a"]
    ...
    def forward(self, x_t, x_t_label, alpha, direction=None, caption=None, caption_emb=None):
        ...
        return self.forward_with_networks(
            x_t, x_t_label, direction, self.vae_enc, self.unet, self.vae_dec, self.sched, self.timesteps, caption_enc, alpha
        )