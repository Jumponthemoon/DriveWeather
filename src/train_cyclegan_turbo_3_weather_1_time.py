...
# def initialize_discriminators(conditions):
#     discriminators = {condition:vision_aided_loss.Discriminator(cv_type='clip', loss_type=args.gan_loss_type, device="cuda") for condition in conditions}

#     for discriminator in discriminators.values():
#         discriminator.cv_ensemble.requires_grad_(False)
#     return discriminators

def initialize_params_disc(conditions,net_disc,net_disc_b):
#     for condition in conditions:
#         print(condition)
#         # print(len(net_disc[condition]))
#     # params_disc = {condition:list(net_disc[condition].parameters()) + list(net_disc_b.parameters()) for condition in conditions}
#     params_disc = {
#     condition: list(net_disc[condition].parameters()) + list(net_disc_b.parameters())
#     for condition in conditions
# }
    params_disc = {}
    for condition in conditions:
        print(len(net_disc[condition]))
        params_disc[condition] = list(net_disc[condition].parameters()) + list(net_disc_b.parameters())

    return params_disc

def initialize_optimizer_disc(conditions,params_disc):

    optimizer_disc = {condition:torch.optim.AdamW(params_disc, lr=args.learning_rate, betas=(args.adam_beta1, args.adam_beta2),
        weight_decay=args.adam_weight_decay, eps=args.adam_epsilon,) for condition in conditions}

    return optimizer_disc

def initialize_lr_scheduler_disc(conditions,optimizer_disc,accelerator):
    lr_scheduler_disc = {condition:get_scheduler(args.lr_scheduler, optimizer=optimizer_disc[condition],
                                        num_warmup_steps=args.lr_warmup_steps * accelerator.num_processes,
                                        num_training_steps=args.max_train_steps * accelerator.num_processes,
                                        num_cycles=args.lr_num_cycles, power=args.lr_power) for condition in conditions}
    return lr_scheduler_disc

def initialize_emb_base(fixed_caption_tgt,tokenizer,text_encoder,conditions):
    
    emb_base = {}
    for condition in conditions:
        token = tokenizer(fixed_caption_tgt[condition],max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt").input_ids[0]
        emb_base[condition] = text_encoder(token.cuda().unsqueeze(0))[0].detach()
...
def main(args):
    ...
    conditions = ['snow','rain','fog']

    if args.gan_disc_type == "vagan_clip":
        # net_disc = initialize_discriminators()
  ...
  #  params_disc = list(net_disc_a.parameters()) + list(net_disc_b.parameters())
    params_disc = initialize_params_disc(conditions, net_disc, net_disc_b)
    ...
    optimizer_disc = initialize_params_disc(conditions,params_disc,net_disc_b)
    ...
    lr_scheduler_disc = initialize_lr_scheduler_disc(conditions,optimizer_disc,accelerator)
    ...
    fixed_caption_tgt = {condition:"Picture of an adverse weather" for condition in conditions}

    # fixed_caption_tgt_night = "Picture of an adverse weather"
    # fixed_caption_tgt_snow = "Picture of an adverse weather"
    # fixed_caption_tgt_rain = "Picture of an adverse weather scene"
    # fixed_caption_tgt_fog = "Picture of an adverse weather scene"

    fixed_a2b_emb_base = initialize_emb_base(fixed_caption_tgt,conditions)
    ...
    unet, vae_enc, vae_dec, *net_disc, net_disc_b = accelerator.prepare(unet, vae_enc, vae_dec, *net_disc, net_disc_b)
...
# )
    ...
    net_lpips, optimizer_gen, *optimizer_disc, train_dataloader, lr_scheduler_gen, lr_scheduler_disc['snow'],lr_scheduler_disc['rain'],lr_scheduler_disc['fog']   = accelerator.prepare(
    net_lpips, optimizer_gen, *optimizer_disc, train_dataloader, lr_scheduler_gen, lr_scheduler_disc['snow'],lr_scheduler_disc['rain'],lr_scheduler_disc['fog'] 
)
    ...
    for condition in conditions:
        for name, module in net_disc[condition].named_modules():
            if "attn" in name:
                module.fused_attn = False    
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
           ...
           # l_acc = [unet, net_disc_snow,net_disc_rain,net_disc_fog, net_disc_b, vae_enc, vae_dec]
            l_acc = [unet, *net_disc, net_disc_b, vae_enc, vae_dec]
            ...
            with accelerator.accumulate(*l_acc):
                ...
                alpha = {'snow':1,'rain':0,'fog':0.5}
                
                train_condition = tgt_weather[0]

                alpha = 1 
                fixed_a2b_emb = fixed_a2b_emb_base[train_condition].repeat(bsz, 1, 1).to(dtype=weight_dtype)
                for c, p in unet.lora_params:
                    p.requires_grad = (c == train_condition)
                ...
                loss_gan_a = net_disc[train_condition](fake_b, for_G=True).mean() * args.lambda_gan
                ...
                optimizer_disc[train_condition].zero_grad()
                ...
                loss_D_A_fake = net_disc[train_condition](fake_b.detach(), for_real=False).mean() * args.lambda_gan
                # if tgt_weather[0] =='snow':
                ...
                if accelerator.sync_gradients:
                    ...
                    params_to_clip = list(net_disc[train_condition].parameters()) + list(net_disc_b.parameters())
                ...
                optimizer_disc[train_condition].step()
                lr_scheduler_disc[train_condition].step()
                optimizer_disc[train_condition].zero_grad()
             ...
             #   loss_D_A_real = net_disc_a(img_b, for_real=True).mean() * args.lambda_gan
                loss_D_A_real = net_disc[train_condition](img_b, for_real=True).mean() * args.lambda_gan
