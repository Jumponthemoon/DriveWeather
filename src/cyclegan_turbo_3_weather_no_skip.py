...
class CycleGAN_Turbo(torch.nn.Module):
    ...
    def forward(self, x_t, x_t_label, alpha,mode=None, direction=None, caption=None, caption_emb=None):
        ...
        if caption_emb is not None:
           ...
           # caption_tokens2 = self.tokenizer('right', max_length=self.tokenizer.model_max_length,
          #          padding="max_length", truncation=True, return_tensors="pt").input_ids.to(x_t.device)

       #     caption_tokens=0.7*caption_tokens+0.3*caption_tokens2
            caption_enc = self.text_encoder(caption_tokens)[0].detach().clone()

          #  caption_enc2 = self.text_encoder(caption_tokens2)[0].detach().clone()  # shape: [batch_size, seq_len, hidden_dim]

# 3. 在嵌入表示上进行加权平均
          #  caption_enc = 0.7 * caption_enc + 0.3 * caption_enc2  # 加权平均后的嵌入表示
