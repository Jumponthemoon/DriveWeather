...
def main(args):
    ...
    if args.gan_disc_type == "vagan_clip":
        ...
        net_discriminators = initialize_discriminators()

# Load discriminators into the accelerator
        net_disc_snow = net_discriminators['snow']
        net_disc_rain = net_discriminators['rain']
  ...
  #  params_disc = list(net_disc_a.parameters()) + list(net_disc_b.parameters())
    ...
    fixed_caption_tgt_snow = "Driving in snow"
    fixed_caption_tgt_rain = "Driving in rain"
           ...
           # l_acc = [unet, net_disc_a, net_disc_b, vae_enc, vae_dec]
            ...
            with accelerator.accumulate(*l_acc):
                ...
                if tgt_weather == 'snow':
                    ...
                    net_disc_a = net_disc_snow
                    optimizer_disc = optimizer_disc_snow
                    lr_scheduler_disc = lr_scheduler_disc_snow
                else:
                    # print(tgt_weather)

                    fixed_a2b_emb = fixed_a2b_emb_base_rain.repeat(bsz, 1, 1).to(dtype=weight_dtype)
                    net_disc_a = net_disc_rain
                    optimizer_disc = optimizer_disc_rain
                    lr_scheduler_disc = lr_scheduler_disc_rain
                ...
                #    params_to_clip = list(net_disc_snow.parameters()) + list(net_disc_b.parameters() + list(net_disc_rain.parameters()) )
