...
class CycleGAN_Turbo(torch.nn.Module):
    def __init__(self, frame_length,mode = None, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_unet=8, lora_rank_vae=4):
        super().__init__()
    ...
    def forward_with_networks(x, x_label, direction, vae_enc, unet, vae_dec, sched, timesteps, text_emb, alpha, total_length, mode, middle_image_attention_features=None):
    ...
    def forward(self, x_t, x_t_label, alpha, total_length, mode=None, direction=None, caption=None, caption_emb=None, middle_image=None):
        ...
        return self.forward_with_networks(
            x_t, x_t_label, direction, self.vae_enc, self.unet, self.vae_dec, self.sched, self.timesteps, caption_enc, alpha, total_length, mode)