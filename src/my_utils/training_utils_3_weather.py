...
class PairedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_img_label = Image.open(os.path.join(self.input_folder, img_name.replace('_frame_camera.png','_gt_labelIds.png')))
        # array1 = np.array(image1)
        # array2 = np.array(image2)

        # # Concatenate the images by channel (axis=2)
        # # Assuming the images are RGB, this will result in an array with 6 channels
        # concatenated_array = np.concatenate((array1, array2), axis=2)

        # # Convert back to an image
        # # Note: The result is no longer a standard RGB image. 
        # # You can save it as an intermediate representation if needed
        # concatenated_image = Image.fromarray(concatenated_array.astype('uint8'))
        ...
        img_t_label = self.T(input_img_label)
        img_t = F.to_tensor(img_t)
        img_t_label = F.to_tensor(img_t_label)
        img_t = torch.cat((image1_tensor, image2_tensor), dim=0)
...
class UnpairedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        tgt_base = os.path.basename(img_path_tgt)
        if 'snow' in tgt_base:
            img_weather = 'snow'
        if 'rain' in tgt_base:
            img_weather = 'rain'
        if 'fog' in tgt_base:
            img_weather = 'fog'
        if 'night' in tgt_base:
            img_weather = 'night'
        img_pil_src = Image.open(img_path_src).convert("RGB")
        img_pil_src_label = Image.open(img_path_src.replace('train_A','train_A_seg').replace('_frame_camera.png','_gt_labelIds.png')).convert("L")
        img_pil_tgt = Image.open(img_path_tgt).convert("RGB")
        img_pil_tgt_label = Image.open(img_path_tgt.replace('train_B','train_B_seg').replace('_rgb_anon_snow.jpg','_gt_labelIds.png').replace('_rgb_anon_rain.jpg','_gt_labelIds.png').replace('_rgb_anon_fog.jpg','_gt_labelIds.png').replace('_rgb_anon_night.jpg','_gt_labelIds.png')).convert("L")


        img_t_src = F.to_tensor(self.T(img_pil_src))
        img_t_src_label = F.to_tensor(self.T(img_pil_src_label))
        img_t_src_cat = torch.cat((img_t_src, img_t_src_label), dim=0)

        img_t_tgt = F.to_tensor(self.T(img_pil_tgt))
        img_t_tgt_label = F.to_tensor(self.T(img_pil_tgt_label))
        img_t_tgt_cat = torch.cat((img_t_tgt, img_t_tgt_label), dim=0)


        img_t_src = F.normalize(img_t_src, mean=[0.5], std=[0.5])
        img_t_tgt = F.normalize(img_t_tgt, mean=[0.5], std=[0.5])

        # img_t_src_cat = F.normalize(img_t_src_cat, mean=[0.5], std=[0.5])
        # img_t_tgt_cat = F.normalize(img_t_tgt_cat, mean=[0.5], std=[0.5])
 
        return {
            "pixel_values_src": img_t_src,
            "pixel_values_src_label": img_t_src_label,

            "pixel_values_tgt": img_t_tgt,
            "pixel_values_tgt_label": img_t_tgt_label,
